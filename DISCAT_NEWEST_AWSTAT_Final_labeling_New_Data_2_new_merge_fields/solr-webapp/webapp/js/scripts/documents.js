...
function addWizardFields(active_core, wizard) {
      ...
      function (data) {
        ...
        $.each(data.fields,
            function (i, item) {
              ...
              if ((item.name != "text")&&(item.name != "_version_")){
  ...
  add_field.die("click")
      ...
      function (event) {
        ...
        //var testtest = $("#document-type", documents_form);
        var selected_test = $("option:selected", "#document-type");

       // var new_entry = '"' + selected.val() + '":"' + wiz_doc.val() + '"';
        if(selected_test.text()=="Create a document")
            var new_entry = '"' + selected.val() + '":"' + wiz_doc.val() +'"';
        else{
           if(selected.val()=="Id")
               var new_entry = '"' + selected.val() + '":"' + wiz_doc.val() +'"';
           else
              var new_entry = '"' + selected.val() + '":{"set":"' + wiz_doc.val() + '"}';
        }
        if (index >= 0) {
                ...
                function (event) {
                  ...
                  if (document_type == "json") {
                   ...
                   // toggles(documents_form, false, false, true, '<add>\n' +
                       ...
                       // '</doc>\n' +
                       // '</add>\n', false);
                       toggles(documents_form, false, false, true, '<delete>\n'+
                       '<id>change.me</id>\n' +
                       '</delete>\n', false);
                    $("#attribs").hide();
                    ...
                    var aa_1 = String(aa).split("?").pop();
		    if(String(aa_1).length<44){
                    $("#wizard-doc", wizard).text(String(aa_1));
		    }
                    // change here
